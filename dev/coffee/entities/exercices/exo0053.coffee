			...
			switch
				when typeof inputs.b isnt "undefined"
					...
					if a is 0
						...
						if r.length>0 then infos.push { type:"warning", text:"L'équation est définie pour $x>#{r.pop().tex()}$"}
				...
				when (typeof inputs.c isnt "undefined") and (typeof inputs.d isnt "undefined")
					...
					if a is 0
						...
						if r1.float() > r2.float() then infos.push { type:"warning", text:"L'équation est définie pour $x>#{r1.tex()}$"}
						else infos.push { type:"warning", text:"L'équation est définie pour $x>#{r2.tex()}$"}
					...
					switch
						when (b is 2) or (b is 4)
							...
							if b is 4
								...
								expr = mM.exec ["x"]
								if a is 0
									infos.push { type:"warning", text:"L'équation est définie pour $x>0$"}
							...
							{ goods, eqTex } = @cas_b12(expr,expr1,expr2,a)
							if a is 0 then infos.push { type:"info", text:"La notation $\\ln^2(#{expr.tex()})$ est un racourci pour $\\left(\\ln(#{expr.tex()})\\right)^2$"}
							if a is 1 then infos.push { type:"info", text:"La notation $\\exp^2(#{expr.tex()})$ est un racourci pour $\\left(\\exp(#{expr.tex()})\\right)^2$"}
							...
							if a is 0
								...
								if mM.alea.dice(1,8)
									...
									if (u>w)
										...
										t = Math.floor -racine2*w # Le floor décal racine expr2 à droite
										racine = mM.exec [-t, w, "/"], { simplify:true }
								infos.push { type:"warning", text:"L'équation est définie pour $x>#{racine.tex()}$"}
